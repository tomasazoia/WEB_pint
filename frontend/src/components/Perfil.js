...
const Perfil = () => {
  ...
  const loadEventosCriados = async (userId) => {
    try {
      const response = await axios.get(`https://pint-backend-5gz8.onrender.com/evento/criador/eventos/${userId}`, {
        headers: {
  ...
  const loadEventosInscritos = async (userId) => {
    try {
      const response = await axios.get(`https://pint-backend-5gz8.onrender.com/participantesevento/funcionario/${userId}/eventos`, {
        headers: {
          'x-auth-token': sessionStorage.getItem('token')
        }
      });
  ...
  const loadPreferencias = (userId) => {
    axios.get(`https://pint-backend-5gz8.onrender.com/userpreferences/list/profile/${userId}`, {
      headers: {
        'x-auth-token': sessionStorage.getItem('token')
      }
    })
  ...
  const loadSubAreas = () => {
    axios.get('https://pint-backend-5gz8.onrender.com/subarea/list', {
      headers: {
        'x-auth-token': sessionStorage.getItem('token')
      }
    })
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const token = sessionStorage.getItem('token');
    axios.put('https://pint-backend-5gz8.onrender.com/user/profileup', formData, {
      headers: {
        'x-auth-token': token
      }
    })
  ...
  const handlePreferencesSubmit = async () => {
    ...
    try {
      const response = await axios.put(`https://pint-backend-5gz8.onrender.com/userpreferences/update/user/${user.ID_FUNCIONARIO}`, updatedPreferences, {
        headers: {
          'x-auth-token': token
        }
      });
