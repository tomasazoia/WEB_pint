...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchEventData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/evento/eventos-by-area');
        const formattedData = [
          ['Área', 'Eventos'],
    ...
    const fetchCommentData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/comentarios_evento/quantidade'); // Chamada ao novo endpoint
        const formattedData = [
          ['Tipo', 'Quantidade'],
    ...
    const fetchCommentLocalData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/comentarios_local/quantidade'); // Chamada ao novo endpoint
        const formattedData = [
          ['Tipo', 'Quantidade'],
