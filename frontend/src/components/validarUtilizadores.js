...
const UsersNaoValidados = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const usersResponse = await axios.get(`http://localhost:3000/user/listnaovalidados/${userResponse.data.ID_FUNCIONARIO}`, {
        headers: {
          'x-auth-token': token
        }
      });
  ...
  const validarUser = async (userId) => {
    try {
      await axios.put(`http://localhost:3000/user/validar/${userId}`);
