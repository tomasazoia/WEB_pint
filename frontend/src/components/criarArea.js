...
const CriarArea = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (selectedArea) {
                ...
                await axios.put(`http://localhost:3000/area/update/${selectedArea.ID_AREA}`, { NOME_AREA: nomeArea }, {
                    headers: {
                        'x-auth-token': token
                    }
                });
                ...
                // Criar nova área
                const response = await axios.post('http://localhost:3000/area/create', { NOME_AREA: nomeArea }, {
                    headers: {
                        'x-auth-token': token
                    }
                });
    ...
    const handleDelete = async (areaId) => {
        try {
            const token = sessionStorage.getItem('token');
            await axios.delete(`http://localhost:3000/area/delete/${areaId}`, {
                headers: {
                    'x-auth-token': token
                }
            });
