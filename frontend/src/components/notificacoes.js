...
const Notificacoes = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const userId = userResponse.data.ID_FUNCIONARIO;
                const response = await axios.get(`http://localhost:3000/notificacoes/user/${userId}`, {
                    headers: {
                        'x-auth-token': token
                    }
                });
    ...
    const marcarComoLida = async (id) => {
        try {
            await axios.put(`http://localhost:3000/notificacoes/read/${id}`);
    ...
    const apagarNotificacao = async (id) => {
        try {
            await axios.delete(`http://localhost:3000/notificacoes/delete/${id}`);
