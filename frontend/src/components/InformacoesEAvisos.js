...
const InformacoesView = () => {
  ...
  const fetchInformacoes = async () => {
    try {
      const response = await axios.get('https://pint-backend-5gz8.onrender.com/infos/list', {
        headers: {
          'x-auth-token': token,
        },
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://pint-backend-5gz8.onrender.com/infos/delete/${id}`, {
        headers: {
          'x-auth-token': token,
        },
      });
  ...
  const handleSave = async () => {
    try {
      if (editingInfo) {
        // Editar Informação
        await axios.put(
          `https://pint-backend-5gz8.onrender.com/infos/update/${editingInfo.ID_INFORMACAO}`,
          newInfo,
          {
            headers: {
              'x-auth-token': token,
        ...
        await axios.post('https://pint-backend-5gz8.onrender.com/infos/create', newInfo, {
          headers: {
            'x-auth-token': token,
          },
        });
