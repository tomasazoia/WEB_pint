...
const ReportsView = () => {
    ...
    const fetchReports = async (userId) => {
        try {
            ...
            const response = await axios.get(`https://pintfinal-backend.onrender.com/reporteventos/list`, {
                headers: {
                    'x-auth-token': token,
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await axios.delete(`https://pintfinal-backend.onrender.com/reporteventos/delete/${id}`, {
                headers: {
                    'x-auth-token': token,
                },
            });
    ...
    const handleCardClick = (idEvento) => {
        navigate(`/evento/get/${idEvento}`); // Redireciona para a página do comentário
    };
        ...
        <div className="container mt-5">
            ...
            <div className="row">
                {reports.map((report) => (
                    <div className="col-md-4" key={report.ID_COMENTARIO}>
                        <Card className="mb-4" onClick={() => handleCardClick(report.comentariosevento.evento.ID_EVENTO)}>
                            <Card.Body>
                            <Card.Text>
                                    <strong>Nome do Evento:</strong> {report.comentariosevento.evento.NOME_EVENTO}
                                </Card.Text>
                                <Card.Text>
                                    <strong>Comentário:</strong> {report.comentariosevento.DESCRICAO}
                                </Card.Text>
                                <Card.Text>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="danger" onClick={() => handleDelete(selectedReport.ID_REPORT)}>
                        Eliminar
                    </Button>
                </Modal.Footer>
            </Modal>
